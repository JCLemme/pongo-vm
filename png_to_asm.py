#!/usr/bin/env python3
"""
Convert PNG images to assembler !Data directives using xterm color palette
"""
...
def convert_png_to_asm(png_path, output_path, palette):
    """
    Convert PNG to assembler !Data directives

    Args:
        png_path: Path to input PNG file
        output_path: Path to output .posm file
    ...
    # Generate label-safe name from filename
    basename = Path(png_path).stem
    label_name = basename.replace('-', '_').replace('.', '_')
    ...
    with open(output_path, 'w') as f:
        ...
        f.write("\n")
        f.write(f"IMG_{label_name}_w: !Data {width}\n")
        f.write(f"IMG_{label_name}_h: !Data {height}\n")
        f.write("\n")
        f.write(f"IMG_{label_name}_data: \n")

        for row in pixel_data:
            # Format as hex bytes
            hex_values = [f"${val:02X}" for val in row]
            f.write("!Data " + ", ".join(hex_values) + "\n")
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python png_to_asm.py <input.png> [output.posm]")
        print("  If output not specified, will use input name with .posm extension")
        sys.exit(1)
    ...
    if len(sys.argv) >= 3:
        ...
        output_path = Path(png_path).with_suffix('.posm')

    # Load palette
    palette = XtermPalette('xterm_colors.json')

    # Convert
    convert_png_to_asm(png_path, output_path, palette)
